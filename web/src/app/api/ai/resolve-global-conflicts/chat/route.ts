...
function flattenActions(prev: any) {
  const out: any[] = [];
  const sugg = Array.isArray(prev?.suggestions) ? prev.suggestions : [];
  for (const s of sugg) {
...
function buildCandidates(args: {
  ...
  // Mantém poucos candidatos para reduzir tokens (evita respostas truncadas).
...
export async function POST(req: Request) {
  try {
    ...
    if (!userMsg) return NextResponse.json({ error: "Mensagem vazia." }, { status: 400 });
    ...
    let schedules: any[] = [];
    if (slotIds.length) {
      const sel = hasActivityType ? "id,time_slot_id,teacher_id,class_id,room_id,activity_type" : "id,time_slot_id,teacher_id,class_id,room_id";
    ...
    const sys =
      ...
      "O usuário está conversando para ajustar as sugestões (mover/excluir regras) e reduzir conflitos. " +
      "Use SOMENTE os slots candidatos fornecidos em candidates para move_rule. " +
      "Considere as ações anteriores (previous_actions) para não desfazer o que já foi sugerido, a menos que o usuário peça. " +
      "Retorne estritamente JSON conforme o schema, sem texto fora do JSON.";

    const prevActionsAll = previous ? flattenActions(previous) : [];
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      const teacherIds = new Set(
        batch
          .map((c: any) => String(c?.requested?.teacherId ?? "").trim())
          .filter((x: string) => x),
      );
      const prevActions = Array.isArray(prevActionsAll)
        ? prevActionsAll.filter((a: any) => teacherIds.has(String(a?.teacher_id ?? "").trim()))
        : [];
      ...
      const userPayload = {
        ...
        outputRules: [
          ...
          "Para cada conflito, sugira no máximo 1 ação move_rule (se houver candidates) OU uma ação note (se não houver).",
          "Não invente IDs; use os IDs de requested.*.",
    ...
    const out = {
      summary:
        batches.length <= 1
          ? String(parts[0] ?? "")
          : `Atualizei as sugestões em ${batches.length} partes.\n` +
            parts
              .map((p, idx) => {
      ...
      meta: { shift, conflicts_sent: compact.length, batches: batches.length, batch_size: batchSize },
    };
