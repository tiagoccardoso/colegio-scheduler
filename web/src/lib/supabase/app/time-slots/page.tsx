...
export default async function Page({
  ...
  async function createAction(formData: FormData) {
    ...
    const { error } = await supabase
      .from("time_slots")
      .insert({ school_id: profile.school_id, weekday, starts_at, ends_at });
  ...
  return (
    <Shell title="Horários">
      <div className="grid gap-4">
        <Flash message={error || msg || (loadError ? loadError.message : null)} />
              ...
              <tbody>
                {(rows as TimeSlot[] | null)?.map((row) => (
                  <tr key={row.id} className="border-t border-zinc-100 dark:border-zinc-900">
                    <td className="px-4 py-3 text-sm text-zinc-800 dark:text-zinc-200">{WEEKDAYS[row.weekday] ?? row.weekday}</td>
                    <td className="px-4 py-3 text-sm text-zinc-800 dark:text-zinc-200">{row.starts_at}</td>
                    <td className="px-4 py-3 text-sm text-zinc-800 dark:text-zinc-200">{row.ends_at}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap items-center gap-2">
                        <details>
                          <summary className="cursor-pointer text-sm font-medium text-zinc-800 dark:text-zinc-200">Editar</summary>
