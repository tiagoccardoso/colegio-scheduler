...
export default async function Page({
  ...
  if (classId) {
    const { data: sched } = await supabase
      .from("schedules")
      .select("id, time_slot_id, subject:subjects(name), teacher:teachers(name), room:rooms(name)")
      .eq("class_id", classId);
    ...
    if (result.error) {
      redirect(
        "/schedule?classId=" +
          ...
          "&error=" +
          encodeMsg("Conflito de horário: " + result.error.message)
      );
    }
  ...
  return (
    <Shell title="Grade" subtitle="Montagem manual">
      <div className="grid gap-4">
          ...
          >
            <label className="grid gap-2">
              ...
              <select
                ...
                className="min-w-[260px] rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:border-zinc-400 dark:border-zinc-800 dark:bg-zinc-950 dark:focus:border-zinc-600"
              >
            ...
            <button
              type="submit"
              className="h-10 rounded-xl bg-zinc-900 px-4 py-2 text-sm font-semibold text-white hover:bg-zinc-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200"
            >
              Carregar
            </button>
          </form>
        ...
        {!classId ? (
          <p className="text-sm text-zinc-600 dark:text-zinc-400">Selecione uma turma para visualizar a grade.</p>
        ) : null}
              ...
              <table className="min-w-[900px] w-full">
                <thead>
                  <tr>
                    ...
                    {WEEKDAYS.map((w) => (
                      <th key={w.key} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                        {w.label}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {periodList.map((p) => (
                    <tr key={p.key} className="border-t border-zinc-100 dark:border-zinc-900">
                      <td className="px-4 py-3 text-sm text-zinc-800 dark:text-zinc-200 whitespace-nowrap">
                        {p.starts_at}–{p.ends_at}
                      </td>

                      {WEEKDAYS.map((w) => {
                        const ts = slotByDayAndPeriod.get(`${w.key}|${p.key}`);
                        if (!ts) return <td key={w.key} className="px-4 py-3 text-sm text-zinc-500 dark:text-zinc-500">—</td>;

                        const sched = scheduleBySlot.get(ts.id);

                        const cellForm = (mode: "Adicionar" | "Editar") => (
                          <details>
                            <summary className="cursor-pointer text-sm font-medium text-zinc-800 dark:text-zinc-200">{mode}</summary>
                            <form action={upsertScheduleAction} className="mt-3 grid gap-3">
                              <input type="hidden" name="class_id" value={classId} />
                              <input type="hidden" name="time_slot_id" value={ts.id} />

                              <select name="subject_id" required defaultValue="" className="rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950">
                                <option value="" disabled>Disciplina...</option>
                                {(subjects as SubjectRow[] | null)?.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                              </select>

                              <select name="teacher_id" required defaultValue="" className="rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950">
                                <option value="" disabled>Professor...</option>
                                {(teachers as TeacherRow[] | null)?.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
                              </select>

                              <select name="room_id" defaultValue="" className="rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950">
                          ...
                          <td key={w.key} className="px-4 py-3 align-top">
                            {sched ? (
                              <div className="grid gap-2">
                                <div>
                                  <div className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">{sched.subject?.name ?? "—"}</div>
                                ...
                                <form action={deleteScheduleAction}>
                                  ...
                                  <input type="hidden" name="class_id" value={classId} />
                                  <ConfirmButton
                                    confirmText="Remover esta aula?"
                                    type="submit"
                                    className="rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm font-medium text-zinc-800 hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200 dark:hover:bg-zinc-900"
                                  >
                                    Remover
                                  </ConfirmButton>
                                </form>
                  ...
                  {periodList.length === 0 ? (
                    <tr className="border-t border-zinc-100 dark:border-zinc-900">
                      <td colSpan={6} className="px-4 py-6 text-sm text-zinc-600 dark:text-zinc-400">
                        Nenhum horário cadastrado em <strong>Horários</strong>. Cadastre os time slots primeiro.
                      </td>
                    </tr>
                  ) : null}
                </tbody>
